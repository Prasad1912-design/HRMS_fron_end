...
const AddEmployee = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (empTypeRes.data.success) setEmpTypes(empTypeRes.data.empTypes);
        if (employeeRes.data.success) setEmployees(employeeRes.data.employees);

        if (isEditMode) {
          const empRes = await api.get(`/auth/getEmployeeById/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        response = await api.put(`/auth/updateEmployee/${id}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        response = await api.post("/auth/registerNewEmployee", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
    ...
    <div className="min-h-screen bg-slate-100 p-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-8 shadow-lg mb-8 text-white">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          ...
          <Link
            to="/admin/employees"
            className="bg-white text-slate-800 px-5 py-3 rounded-xl font-medium flex items-center gap-2"
          >
            <FaArrowLeft />
            Back to Employees
          </Link>
          ...
          <form onSubmit={handleSubmit}>
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full border border-slate-300 rounded-xl py-3 pl-11 pr-4"
                      required
                    />
                  </div>
                </div>
                ...
                {/* SALARY */}
                <div>
                  <label className="block mb-2 font-medium">Monthly Salary</label>
                  <div className="relative">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full border border-slate-300 rounded-xl py-3 px-4"
                    required
                  >
                    <option value="">Select Employment Type</option>
                    {empTypes.filter((data)=>data.status==="active")
                    .map((t) => (
                ...
                {/* MANAGER */}
                <div>
                  <label className="block mb-2 font-medium">Reporting Manager</label>
                  <select
                  ...
                  >
                    ...
                    {employees.filter((emp)=>emp.status==="active" && emp._id !== id && emp.designation.toLowerCase().includes("manager"))
                      .map((e) => (
              ...
              <div className="flex justify-end gap-3 mt-8">
                ...
                <Link
                  to="/admin/employees"
                  className="px-5 py-3 border border-slate-300 rounded-xl"
                >
                  Cancel
                </Link>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-xl flex items-center gap-2"
                >
                  <FaSave />
